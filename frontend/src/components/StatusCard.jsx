...
const WORKFLOW_STAGES = [
  {
    key: 'vision',
    label: 'Vision',
    description: 'Analysing graphs & figures',
    icon: (
...
export default function StatusCard({ jobId, onRetry }) {
  ...
  const handleRetry = async () => {
    ...
    try {
      await onRetry();
      // Reset card so it starts polling the new attempt
    ...
    <div className="mt-3 p-4 rounded-2xl bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 border border-indigo-200 dark:border-indigo-800">
      ...
      <div className="space-y-1.5">
        {WORKFLOW_STAGES.map((stage, index) => {
          ...
          const isActive     = !isCompleted && !isFailed && index === activeIndex;
          const isFailedNode = isFailed && stage.key === currentStep;
          const isPending    = !isCompleted && !isFailed && index > activeIndex;
          ...
          return (
            <div key={stage.key}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className={`text-sm font-medium leading-none ${
                    isFailedNode ? 'text-red-700 dark:text-red-300' :
                    ...
                    {stage.label}
                    {isFailedNode && (
                      <span className="ml-2 text-xs font-normal text-red-500 dark:text-red-400">— failed here</span>
                    )}
                  </div>
              ...
              {/* ── Inline error card beneath the failed node ── */}
              {isFailedNode && (
                <div className="mt-1.5 mx-1 p-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-red-700 dark:text-red-300 mb-0.5">Generation failed</p>
                      {status.error && (
                        <p className="text-xs text-red-600 dark:text-red-400 break-words">{status.error}</p>
                      )}
                    </div>
                  </div>
                  {onRetry && (
                    <button
                      onClick={handleRetry}
                      disabled={retrying}
                      className="mt-2.5 w-full inline-flex items-center justify-center gap-2 px-3 py-1.5 text-xs font-semibold text-white bg-red-600 hover:bg-red-700 active:bg-red-800 disabled:opacity-60 rounded-lg shadow-sm transition-all"
                    >
                      {retrying ? (
                        <>
                          <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
